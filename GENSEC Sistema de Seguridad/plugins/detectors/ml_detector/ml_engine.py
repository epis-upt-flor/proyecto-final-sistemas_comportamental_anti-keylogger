...
class MLEngine:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.prediction_timeout = (
            self.ml_config.get("prediction_timeout_ms", 5000) / 1000.0
        )
        ...
        self.executor = ThreadPoolExecutor(
            max_workers=self.config.get("performance", {}).get(
        ...
        logger.info(
            f"[ML_ENGINE] Inicializado (ONNX: {self.use_onnx}, Modelo: {self.current_model_type})"
        )
        ...
        try:
            ...
            if self.use_onnx:
                if self._load_onnx_model():
                    ...
                    logger.warning(
                        "[ML_ENGINE] ONNX falló, usando sklearn como fallback"
                    )
            ...
            logger.info(
                f"[ML_ENGINE] ✅ Modelo {self.current_model_type} cargado en {self.stats['model_load_time']:.2f}s"
            )
    ...
    def _load_onnx_model(self) -> bool:
        ...
        try:
            ...
            if not onnx_path:
                logger.error(
                    f"[ML_ENGINE] No se encontró modelo ONNX en {self.model_path}"
                )
                return False
            ...
            self.output_names = [
                output.name for output in self.onnx_model.get_outputs()
            ]

            # Validar modelo con predicción de prueba
            test_input = np.random.randn(1, len(self.feature_columns) or 81).astype(
                np.float32
            )
            try:
                _ = self.onnx_model.run(
                    self.output_names, {self.input_name: test_input}
                )
    ...
    def _load_sklearn_model(self) -> bool:
        ...
        try:
            ...
            if not sklearn_path:
                logger.error(
                    f"[ML_ENGINE] No se encontró modelo sklearn en {self.model_path}"
                )
                return False
            ...
            try:
                ...
                logger.info(
                    f"[ML_ENGINE] ✅ Modelo sklearn validado: {sklearn_path.name}"
                )
                return True
            except Exception as e:
            ...
            for classes_file in classes_files:
                ...
                if path.exists():
                    with open(path, "r", encoding="utf-8") as f:
                        ...
                        logger.info(
                            f"[ML_ENGINE] Clases cargadas: {self.label_classes}"
                        )
                        return
            ...
            for metadata_file in metadata_files:
                ...
                if path.exists():
                    with open(path, "r", encoding="utf-8") as f:
                        ...
                        logger.info(
                            f"[ML_ENGINE] Metadatos cargados: {len(self.feature_columns)} características"
                        )
                        return

            logger.warning(
                "[ML_ENGINE] No se encontraron metadatos, usando configuración por defecto"
            )
    ...
    def predict(
        self, features: np.ndarray, async_prediction: bool = False
    ) -> PredictionResult:
        """
        ...
        try:
            ...
            if self.current_model_type == "ONNX":
                ...
                raise PredictionError(
                    f"Tipo de modelo no soportado: {self.current_model_type}"
                )
            ...
            confidence_scores = [
                prob if pred == "Keylogger" else 1.0 - prob
    ...
    def _predict_async(self, features: np.ndarray) -> PredictionResult:
        ...
        try:
            ...
            logger.error(
                f"[ML_ENGINE] Timeout en predicción ({self.prediction_timeout}s)"
            )
    ...
    def _predict_onnx(self, features: np.ndarray) -> Tuple[List[str], List[float]]:
        ...
        try:
            ...
            outputs = self.onnx_model.run(
                self.output_names, {self.input_name: features_float32}
            )
    ...
    def _predict_sklearn(self, features: np.ndarray) -> Tuple[List[str], List[float]]:
        ...
        try:
            ...
            if len(predictions) > 0 and isinstance(predictions[0], (int, np.integer)):
                predictions = [
                    (
                        self.label_classes[pred]
                        if pred < len(self.label_classes)
                        else self.label_classes[0]
                    )
    ...
    def _validate_input(self, features: np.ndarray) -> np.ndarray:
        ...
        if features.shape[1] != expected_features:
            logger.warning(
                f"[ML_ENGINE] Ajustando características de {features.shape[1]} a {expected_features}"
            )
            ...
            if features.shape[1] < expected_features:
                ...
                features = np.pad(
                    features, pad_width, mode="constant", constant_values=0
                )
            else:
                # Truncar
        ...
        with self._lock:
            ...
            if self.stats["avg_prediction_time"] == 0:
                ...
                self.stats["avg_prediction_time"] = (
                    ...
                    + (1 - alpha) * self.stats["avg_prediction_time"]
                )
