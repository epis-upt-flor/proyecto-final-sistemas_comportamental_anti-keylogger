...
class NetworkDetectorPlugin(BasePlugin, DetectorInterface):
    ...
    def _setup_logging(self) -> logging.Logger:
        ...
        if not logger.handlers:
            ...
            file_handler = logging.FileHandler(
                log_dir / f"{self.plugin_name}.log", encoding="utf-8"
            )
        ...
        try:
            ...
            if self.threat_intel:
                try:
                    ...
                    self.logger.warning(
                        f"⚠️ No se pudo actualizar threat intelligence: {e}"
                    )

            # Step 4: Publicar evento
            self._publish_event_sync(
                "network_detector_activated",
                {
        ...
        try:
            ...
            if config_path.exists():
                ...
                self.whitelist_domains = set(
                    whitelist_config.get("trusted_domains", [])
                )
                self.system_processes = set(
                    whitelist_config.get("system_processes", [])
                )

                self.logger.info(
                    f"📋 Configuración cargada: {len(self.config)} secciones"
                )
                return True
            else:
                self.logger.warning(
                    "⚠️ Archivo config.json no encontrado, usando configuración por defecto"
                )
    ...
    def _create_network_connection(
    ...
    async def _analyze_connection_by_strategy(
        self, event_type: str, connection: NetworkConnection
    ):
        """
    ...
    async def _analyze_data_transfer(self, connection: NetworkConnection):
        ...
        if self.pattern_detector:
            ...
            if connection.bytes_sent > 0:
                upload_ratio = connection.bytes_sent / (
                    connection.bytes_sent + connection.bytes_received + 1
                )

                threshold = self.config.get("detection_thresholds", {}).get(
                    "exfiltration_ratio_threshold", 0.8
                )
    ...
    async def _analyze_process_activity(self, connection: NetworkConnection):
        ...
        if self.pattern_detector:
            ...
            beacon_pattern = await self.pattern_detector.detect_beacon_pattern(
                connection.process_name, connection.dest_ip
            )
    ...
    async def _create_threat_detection(
        ...
        try:
            ...
            # Cachear detección
            detection_id = (
                f"{threat_type}_{connection.connection_id}_{int(time.time())}"
            )
    ...
    async def _start_network_monitoring(self):
        ...
        if not self.monitoring_active and self.network_analyzer:
            ...
            self.monitoring_thread = threading.Thread(
                target=self._network_monitoring_loop, daemon=True
            )
    ...
    def _network_monitoring_loop(self):
        ...
        self.logger.info(
            "[NETWORK_DETECTOR] Iniciando monitoreo real de conexiones de red"
        )
        ...
        try:
            while self.monitoring_active:
                ...
                try:
                    ...
                    for conn in net_connections:
                        ...
                        if conn.status == psutil.CONN_ESTABLISHED and conn.raddr:
                            try:
                                ...
                                if conn.pid:
                                    try:
                                        ...
                                        process_info = {
                                            ...
                                            "name": proc.name(),
                                            "exe": (
                                                proc.exe() if proc.exe() else "unknown"
                                            ),
                                        }
                                ...
                                conn_key = (
                                    f"{conn.raddr.ip}:{conn.raddr.port}_{conn.pid}"
                                )
                                ...
                                if conn_key not in tracked_connections:
                                    ...
                                    if self._analyze_suspicious_connection(conn_data):
                                        self._trigger_network_alert(
                                            "suspicious_connection", conn_data
                                        )
                                ...
                                if connection_counts[conn.raddr.ip] > 10:
                                    if conn.raddr.ip not in suspicious_ips:
                                        ...
                                        self._trigger_network_alert(
                                            "multiple_connections_from_ip",
                                            {
                                                "remote_ip": conn.raddr.ip,
                                                "connection_count": connection_counts[
                                                    conn.raddr.ip
                                                ],
                                                "process": process_info,
                                            },
                    ...
                    self.logger.error(
                        f"[NETWORK_DETECTOR] Error escaneando conexiones: {e}"
                    )
        ...
        try:
            ...
            if not self._is_local_ip(remote_ip):
                ...
                if remote_port in suspicious_ports:
                    self.logger.warning(
                        f"[DETECTION] Conexión a puerto sospechoso: {remote_ip}:{remote_port}"
                    )
                    return True
                ...
                if process_name in suspicious_processes:
                    self.logger.warning(
                        f"[DETECTION] Proceso sospechoso con conexión externa: {process_name} -> {remote_ip}"
                    )
                    return True
                ...
                if remote_port == 443:
                    ...
                    if (
                        process_name not in browser_processes
                        and "update" not in process_name
                    ):
                        self.logger.warning(
                            f"[DETECTION] Conexión HTTPS desde proceso no-browser: {process_name}"
                        )
                        return True
    ...
    def _trigger_network_alert(self, alert_type: str, conn_data: Dict):
        ...
        try:
            ...
            if hasattr(self, "event_bus") and self.event_bus:
                ...
                self.logger.info(
                    f"[ALERT] {alert_type}: {conn_data.get('remote_ip')}:{conn_data.get('remote_port')}"
                )

            # Log de la alerta
            self.logger.warning(
                f"[DETECTION] {alert_type}: {conn_data.get('remote_ip')} - {alert_event['description']}"
            )
            ...
            for connection_data in network_data:
                ...
                if detection_result and detection_result.get("is_malicious", False):
                    threat = {
                        ...
                        "threat_type": detection_result.get(
                            "threat_type", "network_anomaly"
                        ),
                        "confidence_score": detection_result.get("confidence", 0.0),
                        "description": detection_result.get(
                            "description", "Actividad de red sospechosa detectada"
                        ),
                        "details": {
                            "source_ip": connection.source_ip,
                            "dest_ip": connection.dest_ip,
                            ...
                            "process_name": connection.process_name,
                            "detection_reason": detection_result.get(
                                "reason", "Patrón anómalo"
                            ),
    ...
    def get_confidence_score(self) -> float:
        ...
        total_confidence = sum(
            detection.confidence for detection in self.threat_cache.values()
        )
    ...
    def update_signatures(self) -> bool:
        ...
        try:
            ...
            if self.pattern_detector:
                ...
                self.logger.info(
                    f"🔄 Patrones de red actualizados: {patterns_loaded} patrones"
                )
    ...
    def get_detection_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "threat_intel_sources": (
                len(getattr(self.threat_intel, "feeds", [])) if self.threat_intel else 0
            ),
            "detection_patterns": (
                len(getattr(self.pattern_detector, "patterns", []))
                if self.pattern_detector
                else 0
            ),
            "plugin_uptime": (
    ...
    def _create_network_connection(self, data: Dict[str, Any]) -> NetworkConnection:
        ...
        return NetworkConnection(
            ...
            connection_id=data.get(
                "connection_id",
                f"{data.get('src_ip', '0.0.0.0')}:{data.get('src_port', 0)}-{data.get('dst_ip', '0.0.0.0')}:{data.get('dst_port', 0)}",
            ),
        )
    ...
    def _prepare_event_subscription(self):
        ...
        try:
            ...
            self.logger.info(
                f"📡 Preparado para suscribirse a {len(self.events_to_subscribe)} tipos de eventos"
            )
