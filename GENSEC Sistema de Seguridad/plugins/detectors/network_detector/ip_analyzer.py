...
class IPAnalyzer:
    ...
    def _is_high_risk_country(self, country: str) -> bool:
        ...
        high_risk_countries = {"CN", "RU", "KP", "IR", "PK", "VN", "TH", "IN"}
        ...
        return country in high_risk_countries or country in high_risk_names
            ...
            if cache_file.exists():
                ...
                for ip, analysis_dict in cached_data.items():
                    try:
                        ...
                        self.logger.warning(
                            f"⚠️ Error cargando entrada de caché para {ip}: {e}"
                        )
                        continue

                self.analysis_cache = valid_cache
                self.logger.info(
                    f"💾 Caché de análisis IP cargado: {len(valid_cache)} entradas"
                )
    ...
    async def save_analysis_cache(self):
        ...
        try:
            ...
            self.logger.info(
                f"💾 Caché de análisis guardado: {len(cache_data)} entradas"
            )
...
def main():
    ...
    async def test_ip_analyzer():
        ...
        if success:
            ...
            print(
                f"   Nodos Tor detectados: {summary['metrics']['tor_nodes_detected']}"
            )
            print(
                f"   VPNs detectadas: {summary['metrics']['vpn_connections_detected']}"
            )
