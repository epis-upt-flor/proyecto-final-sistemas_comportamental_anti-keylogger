...
class EventBus:
    ...
    def unsubscribe(
        self, event_type: str, callback: Callable, subscriber_name: str = "unknown"
    ) -> bool:
        """
        ...
        with self._lock:
            try:
                if callback in self._subscribers[event_type]:
                    ...
                    logger.info(
                        f"📋❌ '{subscriber_name}' desuscrito de '{event_type}'"
                    )
                    return True
                return False
            except Exception as e:
    ...
    def publish(
        ...
        with self._lock:
            try:
                ...
                logger.info(
                    f"📢 Publicando evento '{event_type}' de '{source}' "
    ...
    def _notify_subscribers(self, event: Event, subscribers: List[Callable]):
        ...
        for callback in subscribers:
            try:
                ...
                logger.error(
                    f"❌ Error en callback para evento '{event.event_type}': {e}"
                )
