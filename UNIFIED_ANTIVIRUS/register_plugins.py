...
def register_all_plugins():
    ...
    try:
        from plugins.detectors.keylogger_detector.keylogger_detector import (
            KeyloggerDetector,
        )
...
def get_registered_plugins_info():
    ...
    info = {"detectors": [], "monitors": [], "interfaces": [], "total_count": 0}
    ...
    for category in ["detectors", "monitors", "interfaces"]:
        ...
        if isinstance(plugins, list):
            plugins_dict = {
                plugin: registry.get_plugin_info(plugin)
                for plugin in plugins
                if registry.get_plugin_info(plugin)
            }
        else:
        ...
        for plugin_name, plugin_info in plugins_dict.items():
            plugin_data = {
                "name": plugin_name,
                "class_name": (
                    plugin_info.get("class").__name__
                    if plugin_info.get("class")
                    else "Unknown"
                ),
                "category": category,
                "auto_discovered": plugin_info.get("auto_discovered", False),
                "description": getattr(
                    plugin_info.get("class"), "__doc__", "Sin descripción"
                )
                or "Sin descripción",
            }
