...
class SystemUtils:
        ...
        try:
            return {
                ...
                "memory": {
                    ...
                    "swap": (
                        psutil.swap_memory()._asdict() if psutil.swap_memory() else None
                    ),
                },
                "disk_io": (
    ...
    def get_process_info(pid: int) -> Dict[str, Any]:
        ...
        try:
            ...
            info = {
                ...
                "username": (
                    process.username() if hasattr(process, "username") else "unknown"
                ),
            }
    ...
    def is_virtualized() -> Dict[str, Any]:
        ...
        """
        indicators = {"is_virtual": False, "confidence": 0.0, "indicators": []}
        ...
        try:
                ...
                for indicator in vm_indicators:
                    if indicator in text:
                        indicators["indicators"].append(
                            f"Found '{indicator}' in system info"
                        )
            ...
            if (
                system_info["cpu"]["count_physical"]
                < system_info["cpu"]["count_logical"]
            ):
                if (
                    system_info["cpu"]["count_logical"]
                    % system_info["cpu"]["count_physical"]
                    ...
                    indicators["indicators"].append(
                        "Perfect CPU logical/physical ratio"
                    )
    ...
    def monitor_resources(duration: int = 60, interval: int = 1) -> Dict[str, Any]:
        ...
        cpu_values = [
            m["cpu_percent"] for m in measurements if m["cpu_percent"] is not None
        ]
        memory_values = [
            m["memory_percent"] for m in measurements if m["memory_percent"] is not None
        ]
