...
class RobustAntivirusUI:
    ...
    def on_tab_changed(self, event):
        ...
        try:
            ...
            elif selected_tab == "Keylogger Detector":
                # Actualizar datos del detector de keyloggers
                self.root.after(100, self.refresh_keylogger_data)
    ...
    def create_config_tab(self):
        """Actualizar configuración del detector de keyloggers"""
        try:
            if self.engine and hasattr(self.engine, 'plugin_manager'):
                plugin_manager = self.engine.plugin_manager
                keylogger_plugin = plugin_manager.get_plugin('keylogger_detector')
                
                if keylogger_plugin and hasattr(keylogger_plugin, 'config'):
                    # Actualizar sensibilidad
                    new_sensitivity = self.keylogger_sensitivity.get()
                    keylogger_plugin.config['detection_sensitivity'] = new_sensitivity
                    
                    # Actualizar threshold basado en sensibilidad
                    thresholds = {
                        'low': 0.8,
                        'medium': 0.6,
                        'high': 0.4,
                    ...
                    keylogger_plugin.config['detection_threshold'] = thresholds.get(new_sensitivity, 0.6)
                    
                    messagebox.showinfo("Configuración", f"Sensibilidad cambiada a: {new_sensitivity}")
        except Exception as e:
            print(f"Error actualizando configuración del keylogger detector: {e}")
    
    def clear_keylogger_detections(self):
        """Limpiar la lista de detecciones de keyloggers"""
        try:
            # Limpiar treeview
            for item in self.keylogger_tree.get_children():
                self.keylogger_tree.delete(item)
            
            # Limpiar deque de detecciones
            self.keylogger_detections.clear()
            
            messagebox.showinfo("Limpieza", "Lista de detecciones limpiada")
        except Exception as e:
            messagebox.showerror("Error", f"Error limpiando detecciones: {e}")
    
    def show_keylogger_details(self):
        """Mostrar detalles de la detección seleccionada"""
        try:
            selection = self.keylogger_tree.selection()
            if not selection:
                messagebox.showwarning("Selección", "Por favor selecciona una detección")
                return
            
            item = self.keylogger_tree.item(selection[0])
            values = item['values']
            
            details = f"""
🔍 DETALLES DE DETECCIÓN DE KEYLOGGER

⏰ Tiempo: {values[0]}
🔧 Proceso: {values[1]}
📋 Tipo: {values[2]}
📊 Score: {values[3]}
⚠️ Severidad: {values[4]}
🎯 Acción: {values[5]}

=== INFORMACIÓN ADICIONAL ===
• El detector identificó patrones sospechosos
• Basado en análisis de keyloggers reales
• Score calculado con múltiples factores
• Recomendación: Revisar el proceso manualmente
            """
            
            messagebox.showinfo("Detalles de Detección", details)
        except Exception as e:
            messagebox.showerror("Error", f"Error mostrando detalles: {e}")
    
    def export_keylogger_log(self):
        """Exportar log de detecciones de keyloggers"""
        try:
            filename = filedialog.asksaveasfilename(
                defaultextension=".txt",
                filetypes=[("Archivos de texto", "*.txt"), ("Archivos CSV", "*.csv")],
                title="Exportar Log de Keyloggers"
            )
            
            if filename:
                with open(filename, 'w', encoding='utf-8') as f:
                    f.write("REPORTE DE DETECCIONES DE KEYLOGGERS\n")
                    f.write("=" * 50 + "\n")
                    f.write(f"Generado: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
                    
                    # Estadísticas
                    f.write("ESTADÍSTICAS:\n")
                    f.write(f"Procesos Analizados: {self.keylogger_stats['processes_analyzed']}\n")
                    f.write(f"Keyloggers Detectados: {self.keylogger_stats['keyloggers_detected']}\n")
                    f.write(f"Archivos Sospechosos: {self.keylogger_stats['suspicious_files']}\n")
                    f.write(f"Comportamientos Stealth: {self.keylogger_stats['stealth_behaviors']}\n\n")
                    
                    # Detecciones
                    f.write("DETECCIONES:\n")
                    f.write("-" * 30 + "\n")
                    
                    for detection in self.keylogger_detections:
                        f.write(f"Tiempo: {detection.get('tiempo', 'N/A')}\n")
                        f.write(f"Proceso: {detection.get('proceso', 'N/A')}\n")
                        f.write(f"Tipo: {detection.get('tipo', 'N/A')}\n")
                        f.write(f"Score: {detection.get('score', 'N/A')}\n")
                        f.write(f"Severidad: {detection.get('severidad', 'N/A')}\n")
                        f.write("-" * 20 + "\n")
                
                messagebox.showinfo("Exportación", f"Log exportado a: {filename}")
        except Exception as e:
            messagebox.showerror("Error", f"Error exportando log: {e}")
    
    def refresh_keylogger_data(self):
        """Actualizar datos del detector de keyloggers"""
        try:
            if self.engine and hasattr(self.engine, 'plugin_manager'):
                plugin_manager = self.engine.plugin_manager
                ...
                if keylogger_plugin:
                    ...
                    stats = keylogger_plugin.get_stats()
                    keylogger_specific = stats.get('keylogger_specific', {})
                    
                    # Actualizar labels
                    self.keylogger_processes_label.config(text=str(keylogger_specific.get('processes_analyzed', 0)))
                    self.keylogger_detected_label.config(text=str(keylogger_specific.get('confirmed_keyloggers', 0)))
                    self.keylogger_files_label.config(text=str(keylogger_specific.get('file_pattern_matches', 0)))
                    self.keylogger_stealth_label.config(text=str(keylogger_specific.get('stealth_behaviors', 0)))
                    
                    # Actualizar estado
                    is_active = getattr(keylogger_plugin, 'is_active', False)
                    if is_active:
                        self.keylogger_status_label.config(text="Estado: ✅ Activo")
                        self.keylogger_toggle_btn.config(text="🔴 Desactivar")
                    else:
                        self.keylogger_status_label.config(text="Estado: ⏸️ Desactivado")
                        self.keylogger_toggle_btn.config(text="🟢 Activar")
                    
                    # Actualizar estadísticas internas
                    self.keylogger_stats.update(keylogger_specific)
        except Exception as e:
            print(f"Error actualizando datos del keylogger detector: {e}")
    
    def add_keylogger_detection(self, detection_data):
        """Agregar una nueva detección de keylogger a la UI"""
        try:
            # Agregar a deque
            self.keylogger_detections.append(detection_data)
            
            # Agregar a treeview
            values = (
                detection_data.get('timestamp', ''),
                detection_data.get('process_name', ''),
                detection_data.get('type', ''),
                f"{detection_data.get('risk_score', 0):.2f}",
                detection_data.get('severity', ''),
                detection_data.get('recommended_action', '')
            )
            
            # Insertar al principio de la lista
            item_id = self.keylogger_tree.insert('', 0, values=values)
            
            # Colorear según severidad
            severity = detection_data.get('severity', '').lower()
            if severity == 'critical':
                self.keylogger_tree.set(item_id, 'Severidad', '🔴 Critical')
            elif severity == 'high':
                self.keylogger_tree.set(item_id, 'Severidad', '🟠 High')
            elif severity == 'medium':
                self.keylogger_tree.set(item_id, 'Severidad', '🟡 Medium')
            else:
                self.keylogger_tree.set(item_id, 'Severidad', '🟢 Low')
            
            # Limitar número de elementos mostrados
            children = self.keylogger_tree.get_children()
            if len(children) > 100:  # Mantener solo los últimos 100
                for item in children[100:]:
                    self.keylogger_tree.delete(item)
                    
        except Exception as e:
            print(f"Error agregando detección de keylogger: {e}")

    def create_keylogger_tab(self):
        """Crea la pestaña especializada para el detector de keyloggers"""
        keylogger_frame = ttk.Frame(self.notebook)
        self.notebook.add(keylogger_frame, text="🔍 Keylogger Detector")
        
        # Crear canvas con scroll para la pestaña
        canvas = tk.Canvas(keylogger_frame, bg='white')
        scrollbar = ttk.Scrollbar(keylogger_frame, orient="vertical", command=canvas.yview)
        ...
        # Header del detector de keyloggers
        header_frame = ttk.LabelFrame(scrollable_frame, text="🛡️ Estado del Detector de Keyloggers", padding=10)
        header_frame.pack(fill=tk.X, padx=10, pady=5)
        
        # Estado y estadísticas del detector
        status_frame = ttk.Frame(header_frame)
        status_frame.pack(fill=tk.X, pady=5)
        
        self.keylogger_status_label = ttk.Label(status_frame, text="Estado: Inicializando...", 
                                              font=('Segoe UI', 10, 'bold'))
        self.keylogger_status_label.pack(side=tk.LEFT)
        
        self.keylogger_toggle_btn = ttk.Button(status_frame, text="🔴 Desactivar", 
                                             command=self.toggle_keylogger_detector)
        self.keylogger_toggle_btn.pack(side=tk.RIGHT)
        
        # Estadísticas rápidas
        stats_frame = ttk.LabelFrame(scrollable_frame, text="📊 Estadísticas en Tiempo Real", padding=10)
        ...
        ttk.Label(stats_grid, text="Procesos Analizados:").grid(row=0, column=0, sticky=tk.W, padx=5)
        self.keylogger_processes_label = ttk.Label(stats_grid, text="0", font=('Consolas', 10, 'bold'))
        self.keylogger_processes_label.grid(row=0, column=1, sticky=tk.W, padx=5)
        
        ttk.Label(stats_grid, text="Keyloggers Detectados:").grid(row=1, column=0, sticky=tk.W, padx=5)
        self.keylogger_detected_label = ttk.Label(stats_grid, text="0", font=('Consolas', 10, 'bold'), foreground='red')
        self.keylogger_detected_label.grid(row=1, column=1, sticky=tk.W, padx=5)
        
        ttk.Label(stats_grid, text="Archivos Sospechosos:").grid(row=2, column=0, sticky=tk.W, padx=5)
        self.keylogger_files_label = ttk.Label(stats_grid, text="0", font=('Consolas', 10, 'bold'))
        self.keylogger_files_label.grid(row=2, column=1, sticky=tk.W, padx=5)
        
        ttk.Label(stats_grid, text="Comportamientos Stealth:").grid(row=3, column=0, sticky=tk.W, padx=5)
        self.keylogger_stealth_label = ttk.Label(stats_grid, text="0", font=('Consolas', 10, 'bold'))
        self.keylogger_stealth_label.grid(row=3, column=1, sticky=tk.W, padx=5)
        
        # Configuración del detector
        config_frame = ttk.LabelFrame(scrollable_frame, text="⚙️ Configuración del Detector", padding=10)
        config_frame.pack(fill=tk.X, padx=10, pady=5)
        
        # Sensibilidad
        ttk.Label(config_frame, text="Sensibilidad de Detección:").pack(anchor=tk.W)
        self.keylogger_sensitivity = tk.StringVar(value="medium")
        sensitivity_frame = ttk.Frame(config_frame)
        sensitivity_frame.pack(fill=tk.X, pady=5)
        
        ttk.Radiobutton(sensitivity_frame, text="Baja", variable=self.keylogger_sensitivity, 
                        value="low", command=self.update_keylogger_config).pack(side=tk.LEFT, padx=5)
        ttk.Radiobutton(sensitivity_frame, text="Media", variable=self.keylogger_sensitivity, 
                        value="medium", command=self.update_keylogger_config).pack(side=tk.LEFT, padx=5)
        ttk.Radiobutton(sensitivity_frame, text="Alta", variable=self.keylogger_sensitivity, 
                        value="high", command=self.update_keylogger_config).pack(side=tk.LEFT, padx=5)
        ttk.Radiobutton(sensitivity_frame, text="Paranoica", variable=self.keylogger_sensitivity, 
                        value="paranoid", command=self.update_keylogger_config).pack(side=tk.LEFT, padx=5)
        
        # Detecciones recientes
        detections_frame = ttk.LabelFrame(scrollable_frame, text="🚨 Detecciones Recientes", padding=10)
        detections_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=5)
        
        # Treeview para mostrar detecciones
        columns = ("Tiempo", "Proceso", "Tipo", "Score", "Severidad", "Acción")
        self.keylogger_tree = ttk.Treeview(detections_frame, columns=columns, show="headings", height=10)
        
        # Configurar columnas
        self.keylogger_tree.heading("Tiempo", text="Tiempo")
        self.keylogger_tree.heading("Proceso", text="Proceso")
        self.keylogger_tree.heading("Tipo", text="Tipo Detección")
        self.keylogger_tree.heading("Score", text="Score")
        self.keylogger_tree.heading("Severidad", text="Severidad")
        self.keylogger_tree.heading("Acción", text="Acción")
        
        # Configurar anchos de columnas
        self.keylogger_tree.column("Tiempo", width=120)
        self.keylogger_tree.column("Proceso", width=150)
        self.keylogger_tree.column("Tipo", width=120)
        self.keylogger_tree.column("Score", width=60)
        self.keylogger_tree.column("Severidad", width=80)
        self.keylogger_tree.column("Acción", width=120)
        
        # Scrollbar para treeview
        keylogger_scroll = ttk.Scrollbar(detections_frame, orient="vertical", command=self.keylogger_tree.yview)
        self.keylogger_tree.configure(yscrollcommand=keylogger_scroll.set)
        
        # Pack treeview y scrollbar
        self.keylogger_tree.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        ...
        ttk.Button(action_frame, text="🧹 Limpiar Lista", 
                  command=self.clear_keylogger_detections).pack(side=tk.LEFT, padx=5)
        ttk.Button(action_frame, text="📊 Ver Detalles", 
                  command=self.show_keylogger_details).pack(side=tk.LEFT, padx=5)
        ttk.Button(action_frame, text="💾 Exportar Log", 
                  command=self.export_keylogger_log).pack(side=tk.LEFT, padx=5)
        ttk.Button(action_frame, text="🔄 Actualizar", 
                  command=self.refresh_keylogger_data).pack(side=tk.LEFT, padx=5)
        
        # Pack canvas y scrollbar
        canvas.pack(side="left", fill="both", expand=True)
        scrollbar.pack(side="right", fill="y")
        
        # Inicializar datos del keylogger detector
        self.keylogger_detections = deque(maxlen=1000)  # Mantener últimas 1000 detecciones
    ...
    def update_realtime_metrics(self):
        ...
        try:
            ...
            # 3. Actualizar datos del detector de keyloggers si está visible
            current_tab = self.current_tab.get()
            if current_tab == "Keylogger Detector":
                # Actualizar solo si la pestaña está activa para optimizar rendimiento
                try:
                    self.refresh_keylogger_data()
                except Exception as ke:
                    print(f"Error actualizando keylogger detector: {ke}")
    ...
    def handle_threat_detection_smart(self, threat_data):
        ...
        # Detectar si es una amenaza de keylogger específicamente
        is_keylogger_threat = (
            threat_data.get('type') == 'keylogger_detected' or
            'keylogger' in str(threat_data.get('description', '')).lower() or
            threat_data.get('source') == 'keylogger_detector'
        )
        ...
        if aggregated:
            ...
            if aggregated['count'] == 1:
                ...
                if not self.is_common_false_positive(threat_data):
                    log_level = "CRITICAL" if is_keylogger_threat else "WARNING"
                    self.add_smart_log_entry(log_level, f"Nueva amenaza: {aggregated['key']}")
            else:
