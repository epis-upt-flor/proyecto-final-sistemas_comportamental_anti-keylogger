        ...
        try:
            if self.engine and hasattr(self.engine, "plugin_manager"):
                ...
                if keylogger_plugin and hasattr(keylogger_plugin, "config"):
                    ...
                    keylogger_plugin.config["detection_threshold"] = thresholds.get(
                        new_sensitivity, 0.6
                    )

                    messagebox.showinfo(
                        "Configuración", f"Sensibilidad cambiada a: {new_sensitivity}"
                    )
        except Exception as e:
        ...
        try:
            ...
            if not selection:
                messagebox.showwarning(
                    "Selección", "Por favor selecciona una detección"
                )
                return
...
• Recomendación: Revisar el proceso manualmente
        ...
        try:
            ...
            if filename:
                with open(filename, "w", encoding="utf-8") as f:
                    ...
                    f.write(
                        f"Procesos Analizados: {self.keylogger_stats['processes_analyzed']}\n"
                    )
                    f.write(
                        f"Keyloggers Detectados: {self.keylogger_stats['keyloggers_detected']}\n"
                    )
                    f.write(
                        f"Archivos Sospechosos: {self.keylogger_stats['suspicious_files']}\n"
                    )
                    f.write(
                        f"Comportamientos Stealth: {self.keylogger_stats['stealth_behaviors']}\n\n"
                    )
        ...
        try:
            if self.engine and hasattr(self.engine, "plugin_manager"):
                ...
                if keylogger_plugin:
                    ...
                    # Actualizar labels
                    self.keylogger_processes_label.config(
                        text=str(keylogger_specific.get("processes_analyzed", 0))
                    )
                    self.keylogger_detected_label.config(
                        text=str(keylogger_specific.get("confirmed_keyloggers", 0))
                    )
                    self.keylogger_files_label.config(
                        text=str(keylogger_specific.get("file_pattern_matches", 0))
                    )
                    self.keylogger_stealth_label.config(
                        text=str(keylogger_specific.get("stealth_behaviors", 0))
                    )
    ...
    def create_keylogger_tab(self):
        ...
        scrollbar = ttk.Scrollbar(
            keylogger_frame, orient="vertical", command=canvas.yview
        )
        ...
        header_frame = ttk.LabelFrame(
            scrollable_frame, text="🛡️ Estado del Detector de Keyloggers", padding=10
        )
        header_frame.pack(fill=tk.X, padx=10, pady=5)
        ...
        self.keylogger_status_label = ttk.Label(
            status_frame, text="Estado: Inicializando...", font=("Segoe UI", 10, "bold")
        )
        self.keylogger_status_label.pack(side=tk.LEFT)

        self.keylogger_toggle_btn = ttk.Button(
            status_frame, text="🔴 Desactivar", command=self.toggle_keylogger_detector
        )
        self.keylogger_toggle_btn.pack(side=tk.RIGHT)

        # Estadísticas rápidas
        stats_frame = ttk.LabelFrame(
            scrollable_frame, text="📊 Estadísticas en Tiempo Real", padding=10
        )
        ...
        ttk.Label(stats_grid, text="Procesos Analizados:").grid(
            row=0, column=0, sticky=tk.W, padx=5
        )
        self.keylogger_processes_label = ttk.Label(
            stats_grid, text="0", font=("Consolas", 10, "bold")
        )
        self.keylogger_processes_label.grid(row=0, column=1, sticky=tk.W, padx=5)

        ttk.Label(stats_grid, text="Keyloggers Detectados:").grid(
            row=1, column=0, sticky=tk.W, padx=5
        )
        self.keylogger_detected_label = ttk.Label(
            stats_grid, text="0", font=("Consolas", 10, "bold"), foreground="red"
        )
        self.keylogger_detected_label.grid(row=1, column=1, sticky=tk.W, padx=5)

        ttk.Label(stats_grid, text="Archivos Sospechosos:").grid(
            row=2, column=0, sticky=tk.W, padx=5
        )
        self.keylogger_files_label = ttk.Label(
            stats_grid, text="0", font=("Consolas", 10, "bold")
        )
        self.keylogger_files_label.grid(row=2, column=1, sticky=tk.W, padx=5)

        ttk.Label(stats_grid, text="Comportamientos Stealth:").grid(
            row=3, column=0, sticky=tk.W, padx=5
        )
        self.keylogger_stealth_label = ttk.Label(
            stats_grid, text="0", font=("Consolas", 10, "bold")
        )
        ...
        config_frame = ttk.LabelFrame(
            scrollable_frame, text="⚙️ Configuración del Detector", padding=10
        )
        ...
        ttk.Radiobutton(
            sensitivity_frame,
            text="Baja",
            variable=self.keylogger_sensitivity,
            value="low",
            command=self.update_keylogger_config,
        ).pack(side=tk.LEFT, padx=5)
        ttk.Radiobutton(
            sensitivity_frame,
            text="Media",
        ...
        detections_frame = ttk.LabelFrame(
            scrollable_frame, text="🚨 Detecciones Recientes", padding=10
        )
        ...
        self.keylogger_tree = ttk.Treeview(
            detections_frame, columns=columns, show="headings", height=10
        )
        ...
        keylogger_scroll = ttk.Scrollbar(
            detections_frame, orient="vertical", command=self.keylogger_tree.yview
        )
        ...
        ttk.Button(
            action_frame,
            text="🧹 Limpiar Lista",
            command=self.clear_keylogger_detections,
        ).pack(side=tk.LEFT, padx=5)
        ttk.Button(
            action_frame, text="📊 Ver Detalles", command=self.show_keylogger_details
        ).pack(side=tk.LEFT, padx=5)
        ttk.Button(
            action_frame, text="💾 Exportar Log", command=self.export_keylogger_log
        ).pack(side=tk.LEFT, padx=5)
        ttk.Button(
            action_frame, text="🔄 Actualizar", command=self.refresh_keylogger_data
        ).pack(side=tk.LEFT, padx=5)
        ...
        self.keylogger_detections = deque(
            maxlen=1000
        )  # Mantener últimas 1000 detecciones
            ...
            with self.engine:
                ...
                if self.engine.start_system(categories):
                    ...
                    self.data_queue.put(
                        (
                            "success",
                            "Protección activada - Capturando detecciones reales",
                        )
                    )
    ...
    def handle_threat_detection_smart(self, threat_data):
        ...
        is_keylogger_threat = (
            threat_data.get("type") == "keylogger_detected"
            or "keylogger" in str(threat_data.get("description", "")).lower()
            or threat_data.get("source") == "keylogger_detector"
        )
        ...
        if aggregated:
            ...
            if aggregated["count"] == 1:
                ...
                if not self.is_common_false_positive(threat_data):
                    ...
                    self.add_smart_log_entry(
                        log_level, f"Nueva amenaza: {aggregated['key']}"
                    )
            else:
...
"""
    ...
    def update_metrics_display(self):
        ...
        try:
            ...
            self.status_vars["filtered_events"].set(
                str(self.metrics["filtered_events"])
            )
            ...
            ):  # 10% de probabilidad de actualizar dashboard
