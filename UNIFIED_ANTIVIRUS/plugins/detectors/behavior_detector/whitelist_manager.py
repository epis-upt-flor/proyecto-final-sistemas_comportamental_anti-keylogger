...
class WhitelistManager:
    ...
    def is_whitelisted(self, process_name: str, process_path: str = None) -> bool:
        ...
        if self._is_process_name_whitelisted(process_name):
            ...
            if self.log_access:
                logger.info(f"[WHITELIST] ✅ Proceso permitido (nombre): {process_name}")
        ...
        if process_path and self._is_path_whitelisted(process_path):
            ...
            if self.log_access:
                logger.info(f"[WHITELIST] ✅ Proceso permitido (directorio): {process_name} en {process_path}")
    ...
    def reload_config(self, new_config: Dict[str, Any]):
        ...
        try:
            ...
            logger.info(f"[WHITELIST] Configuración recargada: {len(self.allowed_processes)} procesos, {len(self.trusted_directories)} directorios")
