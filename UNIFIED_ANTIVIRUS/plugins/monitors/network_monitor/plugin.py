...
class NetworkMonitorPlugin(BasePlugin, MonitorPluginInterface):
    ...
    def _check_new_connections(self, current_connections: List[Dict[str, Any]]):
        ...
        for conn in current_connections:
            ...
            if conn_id not in self.known_connections:
                ...
                if hasattr(self, 'logger'):
                    self.logger.info(f"🔗 Nueva conexión: {conn.get('process_name')} -> {conn.get('remote_addr')}")
    ...
    def _alert_suspicious_connection(self, conn: Dict[str, Any]):
        ...
        if hasattr(self, 'logger'):
            self.logger.warning(f"🚨 Conexión sospechosa: {conn.get('process_name')} -> {conn.get('remote_addr')}")
    ...
    def _alert_high_traffic(self, mb_sent: float, mb_recv: float):
        ...
        if hasattr(self, 'logger'):
            self.logger.warning(f"📊 Tráfico alto detectado: {mb_sent:.1f}MB/min enviado, {mb_recv:.1f}MB/min recibido")
    ...
    def _alert_high_connection_count(self, pid: int, count: int):
        ...
        if hasattr(self, 'logger'):
            self.logger.warning(f"🔗 Muchas conexiones: {process_name} ({count} conexiones)")
    ...
    def _initialize_network_snapshot(self):
        ...
        try:
            ...
            if hasattr(self, 'logger'):
                self.logger.info(f"📊 Snapshot inicial: {len(self.known_connections)} conexiones")
        except Exception as e:
            if hasattr(self, 'logger'):
